...
swift run ccusage-gauge --version | tail -n 1 | grep -Fx "$version"
```
...
```text
dist/homebrew-cask/ccusage-gauge-<version>-darwin-arm64.dmg
dist/homebrew-cask/ccusage-gauge-<version>-darwin-x64.dmg
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/ccusage-gauge-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/ccusage-gauge-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/ccusage-gauge-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/ccusage-gauge-${version}-darwin-x64.dmg"
```
...
`../homebrew-tap/Casks/ccusage-gauge.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/ccusage-gauge.rb README.md
git diff --staged --stat
git commit -m "chore: release ccusage-gauge <version>"
...
```bash
brew fetch --cask user/tap/ccusage-gauge
HOMEBREW_NO_GITHUB_API=1 brew audit --cask user/tap/ccusage-gauge
```
...
After pushing `Casks/ccusage-gauge.rb`, require `tacogips/homebrew-tap`'s
`update-api-metadata.yml` workflow to succeed for that commit. Verify the
GitHub Raw `api/cask/ccusage-gauge.json`: its `.version` must equal the release
