...
| --- | --- |
| macOS Apple Silicon | `ccusage-gauge-<version>-darwin-arm64.tar.gz` |
| macOS Intel | `ccusage-gauge-<version>-darwin-x64.tar.gz` |
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
scripts/render-homebrew-formula.sh "$version" /path/to/homebrew-tap/Formula/ccusage-gauge.rb
```
...
brew install user/tap/ccusage-gauge
ccusage-gauge --version
brew test user/tap/ccusage-gauge
```
...
After pushing `Formula/ccusage-gauge.rb`, require
`tacogips/homebrew-tap`'s `update-api-metadata.yml` workflow to succeed for that
commit. Verify the GitHub Raw `api/formula/ccusage-gauge.json`: its
