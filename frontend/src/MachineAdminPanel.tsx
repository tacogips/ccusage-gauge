...
export function MachineAdminPanel(props: {
  ...
  return <>
    ...
    <div class="machine-admin-list">
      <For each={props.machines} fallback={<small>No machines registered.</small>}>{(machine) => {
    ...
    <Show when={props.formOpen}><div class="machine-form">
      ...
      <SourceEditor
        label="Codex session directories"
        field="codexSessionDirs"
        values={props.draft.codexSessionDirs}
        includeDefault={props.draft.includeDefaultCodexDir}
        fieldErrors={props.fieldErrors}
        onValues={(values) => update("codexSessionDirs", values)}
        onIncludeDefault={(value) => update("includeDefaultCodexDir", value)}
      />
      <SourceEditor
        label="Claude configuration directories"
        field="claudeConfigDirs"
        values={props.draft.claudeConfigDirs}
        includeDefault={props.draft.includeDefaultClaudeDir}
        fieldErrors={props.fieldErrors}
        onValues={(values) => update("claudeConfigDirs", values)}
        onIncludeDefault={(value) => update("includeDefaultClaudeDir", value)}
      />
...
function SourceEditor(props: {
  label: string;
  field: "codexSessionDirs" | "claudeConfigDirs";
  values: string[];
  ...
  const removeRow = (index: number) => props.onValues(props.values.filter((_, item) => item !== index));
  return <fieldset class="machine-source-editor">
    <legend>{props.label}</legend>
    <label class="machine-field">
      <input
        aria-label={`Include default ${props.field}`}
        type="checkbox"
        checked={props.includeDefault}
        onChange={(event) => props.onIncludeDefault(event.currentTarget.checked)}
      />
      Include default location
    </label>
    <Index each={props.values}>{(value, index) => <div class="machine-source-row">
      <input
      ...
      />
      <button class="secondary" type="button" onClick={() => removeRow(index)}>Remove</button>
      <Show when={props.fieldErrors?.[`${props.field}[${index}]`]}>{(message) => (
        <small class="machine-warning" role="alert">{message()}</small>
      )}</Show>
    </div>}</Index>
    <button class="secondary" type="button" onClick={() => props.onValues([...props.values, ""])}>Add directory</button>
  </fieldset>;
}
