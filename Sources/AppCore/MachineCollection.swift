...
public actor MachineSnapshotStore {
  ...
  public func selection(
    machineIDs requestedIDs: [String],
    now: Date = Date(),
    requiredCoverageStart: Date? = nil,
    dataDisposition: DashboardDataDisposition = .historical
  ) throws -> MachineSnapshotSelection {
    guard !requestedIDs.isEmpty else { throw MachineSelectionError.invalid }
    let requested = Set(requestedIDs)
    guard requested.count == requestedIDs.count else { throw MachineSelectionError.invalid }
    for id in requestedIDs {
      guard let entry = entries[id] else { throw MachineSelectionError.notFound(id) }
      guard entry.descriptor.enabled else { throw MachineSelectionError.disabled(id) }
    }
    return try aggregateSelection(
      entries: orderedEntries().filter { requested.contains($0.descriptor.id) },
      requested: requestedIDs.joined(separator: ","),
      now: now,
      requiredCoverageStart: requiredCoverageStart,
      dataDisposition: dataDisposition
    )
  ...
  private func aggregateSelection(
    ...
    try aggregateSelection(
      entries: orderedEntries().filter(\.descriptor.enabled),
      requested: "all",
      now: now,
      requiredCoverageStart: requiredCoverageStart,
      dataDisposition: dataDisposition
  ...
  private func aggregateSelection(
    entries enabled: [MachineSnapshotEntry],
    requested: String,
    now: Date,
    requiredCoverageStart: Date?,
    dataDisposition: DashboardDataDisposition
